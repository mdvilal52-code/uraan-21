...
// Filters/type/query arrive as server-rendered props (from the Server
// Component in page.tsx reading its `searchParams` prop) instead of the
// useSearchParams() hook. That hook forces the whole subtree behind a
// Suspense boundary with no useful fallback content, which was blocking
// EVERYTHING on this page — navbar, heading, product grid — from painting
// until client JS fully hydrated. Reading them as props lets the page SSR
// with real content immediately; the URL still updates on navigation
// (router.replace below), which re-renders this Server->Client prop chain.
export default function CollectionsClient({ initialType, initialPrice, initialQuery }: CollectionsClientProps) {
  const typeParam = initialType;
  const priceParam = initialPrice;
        ...
        {filtered.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
              {/* First 4 cards = the first row(s) actually on screen at load
                  (2-col mobile, up to 4-col desktop) — those get fetch
                  priority so the LCP candidate isn't competing equally with
                  every other product photo on the page. */}
              {filtered.map((p, i) => (
