...
export async function PATCH(req: NextRequest) {
  ...
  // Reset failed statuses to uploading so the publish route can re-run them
  const update: Record<string, string> = {};
  if (post.yt_status === 'failed') update.yt_status = 'uploading';
  if (post.fb_status === 'failed') update.fb_status = 'uploading';
  if (post.ig_status === 'failed') update.ig_status = 'uploading';
  ...
  return NextResponse.json({ ok: true, postId, retrying: Object.keys(update) });
}
